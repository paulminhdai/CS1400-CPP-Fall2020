...
public class ArrayStack {
    ...
    /**
     * Initializes an empty stack with size of 10
     */
    public ArrayStack() {
        top = 0;
        store = new int[10];
     ...
     * If the size is less than 1, initializes an empty stack with size of 10.
     * @param size Size of the queue.
     */
    public ArrayStack(int size) {
        if (size < 1) {
            System.out.println("The size must be positive. Created a stack of size 10.");
            store = new int[10];
        } 
        else
            store = new int[size];
